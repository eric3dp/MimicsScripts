...
# consider removing imports below
import ctypes
from PIL import ImageFont

fullname = trimatic.get_project_filename() # get full path and file name of project
modelfolder = fullname.rsplit("\\",2)[1] # split the string into two parts at the last \, basically path and filename, and keep just the middle part (the folder)
newfile = fullname.rsplit("\\",1)[0] + "\\" + modelfolder + "_Base.3mf"  # make the file name for the export the same as the folder
...
trimatic.message_box("Select the parts you'd like to make a base for, then click OK","Cylindrical Base Script")
...
newpart2 = trimatic.wrap(entities= newpart1, gap_closing_distance=1, smallest_detail=1, resulting_offset=0.5)
...
base_cyl_ana = trimatic.create_cylinder_axis(origin_point=cyl_origin,direction=[0,0,1],radius=cyl_radius, height=dim_delta[2])
...
base_cyl = trimatic.remove_undercut(undercut_entity=base_cyl,fitting_entity=newpart2, direction= [0,0,-1])
...
base_cyl_hollow = trimatic.hollow(entities=base_cyl_hollow, hollow_type=trimatic.HollowType.Inside, distance=wall_thickness, reduce=False).get_parent()

cut_plane = trimatic.create_plane_normal_origin([0,0,1],[cyl_x-cyl_radius,cyl_y-cyl_radius,cyl_z])
...
# testing labeling - this works but isn't sized or centered. Also does it with surfaces, not a different object.
